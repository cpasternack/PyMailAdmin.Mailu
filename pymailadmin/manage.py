...
@click.group()
def pymailadmin(cls=flask_cli.FlaskGroup):
  """ Mailu command line
  """
...
def register_stats():
  """ register this server with statistic services.
  """

  if os.path.isfile(app.configMail["STATS_INSTANCE_ID_PATH"]):
    with open(app.configMail["STATS_INSTANCE_ID_PATH"], "r") as handle:
      instance_id = handle.read()
  else:
    instance_id = str(uuid.uuid4())
    with open(app.configMail["STATS_INSTANCE_ID_PATH"], "w") as handle:
      handle.write(instance_id)
  if app.configMail["STATS_ENABLE"]:
    try:
      socket.gethostbyname(app.configMail["STATS_ENDPOINT"].format(instance_id))
    except:
      pass
