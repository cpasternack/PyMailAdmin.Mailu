...
class Domain(Base):
  ...
  @property
  def dkim_key(self):
    file_path = app.configMail["MAIL_DKIM_PATH"].format(
      domain=self.name, selector=app.configMail["MAILDKIM_SELECTOR"])
  ...
  def dkim_key(self, value):
    file_path = app.configMail["MAIL_DKIM_PATH"].format(
      domain=self.name, selector=app.configMail["MAIL_DKIM_SELECTOR"])
  ...
  def check_mx(self):
    try:
      hostnames = app.configMail['MAIL_HOSTNAMES'].split(',')
      return any(
...
class Email(object):
  ...
  def sendmail(self, subject, body):
    ...
    from_address = "{0}@{1}".format(
      app.configMail['MAIL_POSTMASTER'],
      idna.encode(app.configMail['MAIL_DOMAIN']).decode('ascii'),
    )
    with smtplib.SMTP(app.configApp['HOST_SMTP'], port=10025) as smtp:
  ...
  def resolve_domain(cls, email):
    """ resolve domain class method by email """
  ...
  def resolve_destination(cls, localpart, domain_name, ignore_forward_keep=False):
    ...
    if app.configMail('MAIL_RECIPIENT_DELIMITER') in localpart:
      localpart_stripped = localpart.rsplit(app.configMail('MAIL_RECIPIENT_DELIMITER'), 1)[0]
...
class User(Base, Email):
  ...
  def get_password_context(self):
    return context.CryptContext(
      ...
      default=self.scheme_dict[app.configApp['APP_PASSWORD_SCHEME']],
    )
  ...
  def send_welcome(self):
    if app.configMail["MAIL_WELCOME"]:
      self.sendmail(app.configMail["MAIL_WELCOME_SUBJECT"],
        app.configMail["MAIL_WELCOME_BODY"])
